...
<div class='container mt-5 d-flex'>
  <aside class="col-sm-3">
   ...
   <form action="<c:url value="/view/quesModify.do"/>" method="post" name="ques_input_form">
        ...
        <input type="hidden" name="qid" value="${quesinfo.qid}"/>
		...
		<select class="form-select form-select-sm" id="ques_option" name="ques_option">
            <c:forEach var="opspec" items="${opspec}">
              <option value="${opspec.name()}" ${opspec.name()==quesinfo.ques_option ? 'selected' : ''}>${opspec.value}</option>
            </c:forEach>
        </select>
        <input class="form-control mt-2" type="text" id="q_subject" name="q_subject" value="${quesinfo.q_subject}" autofocus>

        <textarea class="form-control mt-2" name="q_contents" id="q_contents" style="height:400px;">${quesinfo.q_contents}</textarea>

        <input class="form-control mt-2" type="text" id="q_writer" name="q_writer" value="${sessionScope.loginDto.id}" placeholder="${sessionScope.loginDto.id}" readonly>
		<input class="form-control mt-2" type="text" id="reg_date" name="reg_date" value="${quesinfo.reg_date}" placeholder="${quesinfo.reg_date}" readonly>
		...
		<c:if test="${qDto.ques_state!='문의사항 확인중'}">
			<button<%--  href="<c:url value='/view/quesModify.do?viewPage=${paDto.viewPage}&cntPerPage=${paDto.cntPerPage}&q_writer=${sessionScope.loginDto.id}'/>" --%>
		                       class = "btn btn-sm btn-success m-3">수정</button>
		</c:if>
		
		 <a href="<c:url value='/view/questionRemove.do?qid=${quesinfo.qid}&viewPage=${paDto.viewPage}&cntPerPage=${paDto.cntPerPage}&q_writer=${sessionScope.loginDto.id}'/>"
